...
{
    public class ImportadorUsuarios : Importador
    {
        public ImportadorUsuarios(ILogger<Importador> logger) : base(logger)
        {
        }
        ...
        protected override string GetFicheroTipo() => "Usuarios";

        protected override string GetHeader() => "CreatedById;Usuario;TIPO";

        protected override string GetTableName() => "IN_USUARIOS";

        protected override string GetTableName2() => "";

        private Dictionary<string, Dictionary<string, int>> MasterKey;

        public void ExecuteUsuariosOrigen(Database database, string fullName, DateTime fechaFichero, bool Transaction = false)
        {
