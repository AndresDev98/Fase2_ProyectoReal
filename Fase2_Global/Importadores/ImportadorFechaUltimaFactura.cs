...
{
    public class ImportadorFechaUltimaFactura : Importador
    {
        public ImportadorFechaUltimaFactura(ILogger<Importador> logger) : base(logger)
        {
        }
        ...
        protected override string GetFicheroTipo() => "Fecha Ultima Factura";

        protected override string GetHeader() => "id_cuenta;numero_susc;f_desde;f_hasta";

        protected override string GetTableName() => "IN_FECHA_ULTIMA_FACTURA";

        protected override string GetTableName2() => "";

        private Dictionary<string, Dictionary<string, int>> MasterKey;

        public void ExecuteFechaUltFactura(Database database, string fullName, DateTime fechaFichero, bool Transaction = false)
        {
