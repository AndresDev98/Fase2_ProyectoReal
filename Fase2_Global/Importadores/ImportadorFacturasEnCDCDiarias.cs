...
{
    public class ImportadorFacturasEnCDCDiarias : Importador
    {
        public ImportadorFacturasEnCDCDiarias(ILogger<Importador> logger) : base(logger)
        {
        }

        protected override char GetDelimited() => ';';

        protected override int GetFicheroTipoId() => 4;

        protected override string GetFicheroTipo() => "Facturas Recibidas CDC";

        protected override string GetHeader() => "Número de Factura Fiscal;Fecha Emisión Factura;Fecha Carga en CDC;Fichero csv - Carga;";

        protected override string GetTableName() => "IN_CDC";
        protected override string GetTableName2() => "";

        public void ExecuteFacturasEnCDCDiarias(Database database, string fullName, DateTime fechaFichero, bool Transaction = false)
        {
